...
<br>
<footer class="footer mt-auto py-3 text-muted ${UI.footer_style_class}" style="${UI.fixed_footer ?'position: fixed; ': ''}left: 0; bottom: 0; width: 100%; color: white; z-index: 9999;${UI.hide_footer ? ' display:none;': ' display:block;'}"> <div class="container" style="width: auto; padding: 0 10px;"> <p class="float-end"> <a href="#">Trở về đầu trang</a> </p> ${UI.credit ? '<p>Tạo bởi <svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-star-fill" fill="yellow" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M3.612 15.443c-.396.198-.824-.149-.746-.592l.83-4.73-3.522-3.356c-.329-.314-.158-.888.283-.95l4.898-.696L8 .792l2.191 4.426 4.898.696c.441.062.612.636.283.95l-3.522 3.356.83 4.73c.078.443-.35.79-.746.592L8 13.187l-4.389 2.256z"/></svg> <a href="https://bankhonggioi.com" target="_blank">BKOG</a>.</p>' : ''} <p>© ${UI.copyright_year} - <a href=" ${UI.company_link}" target="_blank"> ${UI.company_name}</a>.</p> </div> </footer>
  `;
	$('body').html(html);
}
...
<button class="btn ${UI.search_button_class}" onclick="if($('#search_bar_form>input').val()) $('#search_bar_form').submit();" type="submit">Tìm kiếm</button>
</form>
</div>
</div>
</nav>
